...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) { // 以“classpath*:”开头
			// 默认是AntPathMatcher，判断“classpath*:”后面字符串有没有 *、？、{} 三种通配符
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			if (rootDirUrl.getProtocol().startsWith(ResourceUtils.URL_PROTOCOL_VFS)) { // VFS类型
				result.addAll(VfsResourceMatchingDelegate.findMatchingResources(rootDirUrl, subPattern, getPathMatcher()));
			} else if (ResourceUtils.isJarURL(rootDirUrl) || isJarResource(rootDirResource)) {  // JAR类型
